...
?>
            <main class="flex-1 max-w-[1280px] mx-auto w-full px-4 lg:px-40 py-8 space-y-12">
                ...
                <section class="space-y-4">
                    <h2 class="text-2xl font-black">New Arrivals</h2>
                    <div class="flex gap-3 overflow-x-auto">
                        <button class="filter-btn bg-primary text-white px-5 py-2 rounded-full" data-category="all">
                            All Products
                        </button>
                        <?php foreach ($categories as $category): ?>
                            <button class="filter-btn bg-gray-200 dark:bg-gray-700 px-5 py-2 rounded-full"
                ...
                <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div class="product-card flex flex-col bg-white dark:bg-[#1e2329] border rounded-xl overflow-hidden"
                        data-category="<?= $product->getCategory_id() ?>">
                        <a href="/Product?id=<?= $product->getId() ?>" class="h-64 bg-gray-200 bg-center bg-no-repeat bg-cover transition-transform duration-500 group-hover:scale-110"
                        ...
                        <div class="p-5 flex flex-col flex-1">
                            <p class="font-bold"><?= $product->getName() ?></p>
                            <div class="mt-auto flex justify-between items-center">
                                <span class="font-black">$<?= $product->getPrice() ?></span>
